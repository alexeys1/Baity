...
public class PrismBreak {
    ...
    private static final HashMap<Long, Float> SMOOTH_PROGRESS = new HashMap<>();
    ...
    public static void render(LevelRenderer levelRenderer, LevelRenderState levelRenderState) {
        ...
        if (!levelRenderState.blockBreakingRenderStates.isEmpty()) {
            try (var _ = levelRenderer.collectPerFrameGizmos()) {
                for (BlockBreakingRenderState state : levelRenderState.blockBreakingRenderStates) {
                    ...
                    long key = pos.asLong();
                    int target = state.progress();
                    Float stored = SMOOTH_PROGRESS.get(key);
                    float smooth = stored == null ? target : stored + (target - stored) * 0.2f;
                    SMOOTH_PROGRESS.put(key, smooth);
                    seen.add(key);
                    float phase = (float) ((now / 1000.0) * (speed * 0.5));
                    float posOffset = (float) ((Math.floorMod(key, 100)) * 0.01);
                    float reach = Math.min(1.0f, (smooth + 1) / 10.0f);
        ...
        SMOOTH_PROGRESS.keySet().removeIf(k -> !seen.contains(k));
    }
    ...
    private static void drawArm(double x0, double y0, double z0, double x1, double y1, double z1,
        ...
        int packed = Mth.hsvToRgb(hue, saturation, lightness);
        int color = ARGB.color(255, ARGB.red(packed), ARGB.green(packed), ARGB.blue(packed));
        Gizmos.line(from, to, color, edgeWidth).persistForMillis(50);
    }
